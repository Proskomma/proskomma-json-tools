...
const styleAsCSS = options => {
    ...
    for (const option in options) {
        cssResult.push(`/* ${option} CSS format : */\n`);
        for (const op in options[option]) {
            if (op.includes(":")) {
                const newOp = op.replace(":", "_");
                cssResult.push(`  .${option}_${newOp
                    .replace(/([A-Z])/g, "_$1")
                    .toLowerCase()} {\n`);
            } else {
                cssResult.push(`  .${option}_${op
                    .replace(/([A-Z])/g, "_$1")
                    .toLowerCase()} {\n`);
            }
            ...
            for (const prop in styles) {
                for (const p in prop) {
                    if (prop[p] === prop[p].toUpperCase()) {
                        ...
                        cssResult.push(`      ${newProp}: ${styles[prop]};\n`);
                    }
                }
            }
            cssResult.push("  }\n");
...
function convertCssToReactNativeStyle(styleSheet) {
    //  note that this function is not exhaustive and needs further work. Unfortunately not all css
