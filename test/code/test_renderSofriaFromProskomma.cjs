...
test(`Render SOFRIA via identity actions (${testGroup})`, async function (t) {
  try {
    t.plan(4);
    const usfm = fse
      .readFileSync(
        path.resolve(path.join("test", "test_data", "usfms", "webbe_mrk.usfm"))
      )
      .toString();
...
test(`Render 1 chapter of SOFRIA (${testGroup})`, async function (t) {
  try {
    t.plan(3);
    const docId = pk.gqlQuerySync("{documents { id } }").data.documents[0].id;
    const cl = new SofriaRenderFromProskomma({
    ...
    const output = {};
    t.doesNotThrow(() =>
      cl.renderDocument({ docId, config: { chapters: ["2"] }, output })
    );
    // console.log(JSON.stringify(output, null, 2));
    const validator = new Validator();
    const validation = validator.validate(
...
test(`Render SOFRIA with atts via identity actions (${testGroup})`, async function (t) {
  try {
    t.plan(3);
    const pk2 = new Proskomma();
    const usfm = fse
      .readFileSync(
        path.resolve(path.join("test", "test_data", "usfms", "ult_uw_mrk.usfm"))
      )
      .toString();
...
test(`Render SOFRIA with multi-para verses (${testGroup})`, async function (t) {
  try {
    t.plan(3);
...
test(`Check for xrefs and wj in SOFRIA (${testGroup})`, async function (t) {
  try {
    t.plan(5);
...
test(`Handle conjunction of note and straddle para for SOFRIA (${testGroup})`, async function (t) {
  try {
    const usxLeaves = [
      "sofria_note",
      "sofria_verse_straddles_para",
      "sofria_note_plus_verse_straddles_paras",
    ];
    ...
    for (const usxLeaf of usxLeaves) {
      ...
      const docId = pk5.gqlQuerySync("{documents { id } }").data.documents[0]
        .id;
      const cl = new SofriaRenderFromProskomma({
        proskomma: pk5,
        actions: identityActions,
...
test(`SOFRIA rems (${testGroup})`, async function (t) {
  try {
    t.plan(3);
    const usfm = fse
      .readFileSync(
        path.resolve(path.join("test", "test_data", "usfms", "rems.usfm"))
      )
      .toString();
...
test(`Handle alternative chapter/verse for SOFRIA (${testGroup})`, async function (t) {
  try {
...
test(`Render francl SOFRIA via identity actions (${testGroup})`, async function (t) {
  try {
    t.plan(4);
    const pk6 = new Proskomma();
    const usfm = fse
      .readFileSync(
        path.resolve(
          path.join("test", "test_data", "usfms", "eng_francl_mrk.usfm")
        )
      )
      .toString();
    ...
    t.ok(validation.isValid);
    t.equal(validation.errors, null);
    const cl2 = new SofriaRenderFromJson({
      srcJson: output.sofria,
      actions: identityActions,
    });
    const output2 = {};
    t.doesNotThrow(() =>
      cl2.renderDocument({ docId: "", config: {}, output: output2 })
    );
  } catch (err) {
    console.log(err);
  }
});
test(`Getting only one Chapter to render (${testGroup})`, async function (t) {
  try {
    const docId = pk.gqlQuerySync("{documents { id }}").data.documents[0].id;
    let currentChapterContext =
      pk.gqlQuerySync(`{document(id: "${docId}") {cIndex(chapter: 1) {
                startBlock
                endBlock
              }}}`);
    ...
    t.equal(
      ...
      currentChapterContext.data.document.cIndex.endBlock + 1,
      "The number of block paragraph render is 1 "
    );
  } catch (err) {
    console.log(err);
  }
...
test("Is there an error when asking a chapter that is not here", async function (t) {
  ...
  try {
    ...
    t.equal(
      err.message,
      "Chapter '1000' not found in document",
...
test(`Getting only multiple Chapter to render (${testGroup})`, async function (t) {
  try {
    const docId = pk.gqlQuerySync("{documents { id }}").data.documents[0].id;
    const renderer = new SofriaRenderFromProskomma({
      proskomma: pk,
    ...
    numberOfBlocks +=
      currentChapterContext.data.document.cIndex.endBlock -
      currentChapterContext.data.document.cIndex.startBlock +
      1;
    currentChapterContext =
      pk.gqlQuerySync(`{document(id: "${docId}") {cIndex(chapter: 3) {
                startBlock
                endBlock
              }}}`);
    numberOfBlocks +=
    ...
    t.equal(
      output.paras.filter((b) => b.type === "paragraph").length,
      numberOfBlocks,
      "The number of block paragraph render is 20 "
    );
  } catch (err) {
    console.log(err);
  }
});
test(`Chapter eating in option (${testGroup})`, async function (t) {
  try {
    const docId = pk.gqlQuerySync("{documents { id }}").data.documents[0].id;
    const renderer = new SofriaRenderFromProskomma({
      proskomma: pk,
    ...
    t.equal(
      ...
      2,
      "The number of chapter in config object is 2"
    );
  } catch (err) {
    console.log(err);
  }
});
test(`Render tr/tc/th via SOFRIA (${testGroup})`, async function (t) {
  try {
    t.plan(5);
    const pk = new Proskomma();
    const usfm = fse
      .readFileSync(
        path.resolve(path.join("test", "test_data", "usfms", "table.usfm"))
      )
      .toString();
    pk.importDocument({ lang: "eng", abbr: "web" }, "usfm", usfm);
    const docId = pk.gqlQuerySync("{documents { id } }").data.documents[0].id;
    ...
    t.equal(
      ...
      numberOfRows,
      `The number of row is not ${numberOfRows}`
    );
    t.equal(
      output.paras
        .filter((b) => b.type === "row")[2]
        .content[0].content.filter((c) => c.subtype === "cell").length,
      numberOfCells,
      `The number of cells render in the 2th row is not ${numberOfCells} `
    );
    const validator = new Validator();
    const validation = validator.validate(
      "constraint",
...
test(`test View (${testGroup})`, async function (t) {
  try {
    const config = {
      showWordAtts: false,
      showTitles: false,
    ...
    const usfm = fse
      .readFileSync(
        path.resolve(path.join("test", "test_data", "usfms", "titus.usfm"))
      )
      .toString();
    pk.importDocument({ lang: "eng", abbr: "web" }, "usfm", usfm);
    const docId = pk.gqlQuerySync("{documents { id } }").data.documents[0].id;
...
test(`Multiple para in verse Start End events in (${testGroup})`, (t) => {
  ...
  try {
    ...
    const usfm = fse
      .readFileSync(
        path.resolve(
          path.join("./", "test", "test_data", "usfms", "startEndVerse.usfm")
        )
      )
      .toString();
    pk6.importDocument({ lang: "eng", abbr: "francl" }, "usfm", usfm);
      ...
      startWrapper: [
        {
          ...
          test: ({ context }) =>
            context.sequences[0].element.subType === "verses",
          action: ({ config, context, workspace, output }) => {
...
test(`Empty milestone events (${testGroup})`, (t) => {
  ...
  try {
    ...
    const usfm = fse
      .readFileSync(
        path.resolve(
          path.join("./", "test", "test_data", "usfms", "empty_milestone.usfm")
        )
      )
      .toString();
    pk7.importDocument({ lang: "ara", abbr: "xxa" }, "usfm", usfm);
    const result = pk7.gqlQuerySync(
      "{documents { id mainSequence {blocks {dump } } } }"
    ).data.documents[0];
...
test(`Weird uW milestone (${testGroup})`, async function (t) {
  try {
    ...
    const usfm = fse
      .readFileSync(
        path.resolve(path.join("test", "test_data", "usfms", "51-PHP.usfm"))
      )
      .toString();
    ...
    t.doesNotThrow(() =>
      cl.renderDocument({
        docId,
        config: { renderers, selectedBcvNotes: [] },
        output,
      })
    );
  } catch (err) {
...
test(`Weird ACT uW milestone (${testGroup})`, async function (t) {
  const chapterLabels = (o) =>
    [...o.paras.matchAll(/<span class="marks_chapter_label">\d+<\/span>/g)]
      .map((i) => i[0])
  ...
  try {
    ...
    const usfm = fse
      .readFileSync(
        path.resolve(path.join("test", "test_data", "usfms", "ULT_ACT.usfm"))
      )
      .toString();
    pk2.importDocument({ lang: "eng", abbr: "web" }, "usfm", usfm);
    const docId = pk2.gqlQuerySync("{documents { id } }").data.documents[0].id;
    ...
    t.doesNotThrow(() =>
      cl.renderDocument({
        docId,
        config: { renderers, selectedBcvNotes: [], chapters: ["5", "10"] },
        output,
      })
    );
